...
public:
    ...
    /**
     * @brief Called *in the DAQ thread* when a frame is assigned its timestamp.
     *
     * This callback is executed for each frame that gets assigned a timestamp, with the timestamp
     * of the computer's clock when the frame was - probably - acquired as first parameter ("master timestamp")
     * and the adjusted timestamp generated by the driver or device as second parameter.
     * The timestamps may already have been preprocessed a bit, so you may not get completely "raw" timestamps.
     * The "master timestamp" may be modified by the callee, it will be set as the frame's new timestamp.
     */
    void setOnFrameTimestamp(std::function<void(milliseconds_t &, const milliseconds_t &, void *)> callback, void *udata = nullptr);
