...
static bool debugLoggingRequested() noexcept
{
    static const bool requested = std::getenv("MINISCOPE_DEBUG") != nullptr;
    return requested;
}

LogCategory::LogCategory(const char *name_, LogSeverity defaultSeverity) noexcept
    : name(name_),
      threshold(static_cast<int>(debugLoggingRequested() ? LogSeverity::Debug : defaultSeverity)),
      next(nullptr)
{
...
{
    const LogMessage lm{sev, cat.name, std::string_view(message)};
    detail::g_handler(lm);
}
